...
def linear_equation(coordsY, coordsX):
    ...
    gradients = gradient_calculator(coordsY,coordsX,base) #this utilises the gradient_calculator function to calculate the linear gradient
...
def equation_obtain(coordsY, coordsX, category: str,Base: int, Repeat_Count: int): #the base is the highest power, Category determines which equation function shall be used
    ...
    if category == "poly":
        ...
        while Repeat_Count != Base:
            ...
            X_subtract = MagicBox.Container(list(map(lambda x:(x**Base)*CoefficientFirst,coordsX))) #Substitutes all X values into the first term
            ...
            if gradient_calculator(Y_new, coordsX)[0] == 0 and Base == 1: #this checks if the previous terms power was 1 and the gradient of the values is 0 (as they should be the same)
